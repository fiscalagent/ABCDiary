...
const HELP_CONTENT: HelpSection[] = [
  {
    ...
    cards: [
      ...
      },
      {
        title: 'Настроение',
        steps: [
          'Значок <strong>😊</strong> в шапке главного экрана — календарь настроения; <strong>+</strong> → <em>Настроение</em> — новая запись.',
          'Оцените утро, день и вечер по шкале 0–10 — у каждого времени суток свой комментарий.',
          'Укажите лекарства и дозу — до трёх, второе и третье под «Ещё лекарства».',
          'Одна запись на день: возврат к тому же дню дополняет её, а не создаёт новую.',
        ],
      },
      {
        title: 'Цели',
        steps: [
          'Значок <strong>🎯</strong> в шапке главного экрана открывает список целей.',
          'Цель — это название, горизонт (месяц/неделя/день), дедлайн и заметка. У месячной цели могут быть недельные или дневные подзадачи, у недельной — только дневные.',
          'Дедлайн можно переносить в любой момент — это не ошибка, а часть мягкого планирования.',
          'Из цели — «Запланировать как дело на сегодня»: появится связанная запись в «Делах».',
        ],
      },
    ],
  },
  {
    ...
    cards: [
      ...
      },
      {
        ...
        steps: [
          'Откройте таблицу — листы <strong>«Эмоции»</strong>, <strong>«Дела»</strong>, <strong>«Цели»</strong> и <strong>«Настроение»</strong> появляются по мере того, чем пользуется пациент.',
